...
func TestStaticAssetsAndCSPUseExternalCSSAndJS(t *testing.T) {
	...
	if !strings.Contains(assetRR.Body.String(), "/assets/base.css") {
		t.Fatalf("asset body missing expected CSS:\n%s", assetRR.Body.String())
	}
	...
	if !strings.Contains(body, `href="/assets/app.css"`) || !strings.Contains(body, `src="/assets/app.js"`) {
