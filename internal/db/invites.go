...
const inviteColumns = `i.id, i.token_hash, COALESCE(i.token_prefix, ''), COALESCE(i.token_encrypted, ''), COALESCE(i.label, ''), i.template_id, t.name,
		       i.expires_at, i.max_uses, i.uses, i.enabled, i.user_expiry_days, i.last_used_at, i.deleted_at, i.created_at, i.updated_at`
...
func (s *Store) CreateInvite(ctx context.Context, invite Invite) (int64, error) {
	...
	result, err := s.db.ExecContext(ctx, `
		INSERT INTO invites (token_hash, token_prefix, token_encrypted, label, template_id, expires_at, max_uses, uses, enabled, user_expiry_days, created_by_user_id, created_at, updated_at)
		VALUES (?, ?, NULLIF(?, ''), ?, ?, ?, ?, 0, 1, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
	`, invite.TokenHash, invite.TokenPrefix, encryptedToken, invite.Label, invite.TemplateID, invite.ExpiresAt, invite.MaxUses, invite.UserExpiryDays, invite.CreatedByUserID)
	if err != nil {
		return 0, err
	}
...
func (s *Store) InvitePreview(ctx context.Context, limit int) ([]Invite, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, COALESCE(label, ''), expires_at, max_uses, uses, enabled, user_expiry_days, deleted_at
		FROM invites
	...
	for rows.Next() {
		var invite Invite
		if err := rows.Scan(&invite.ID, &invite.Label, &invite.ExpiresAt, &invite.MaxUses, &invite.Uses, &invite.Enabled, &invite.UserExpiryDays, &invite.DeletedAt); err != nil {
			return nil, err
		}
		invites = append(invites, invite)
	}
...
func (s *Store) ReserveInviteUse(ctx context.Context, inviteID int64, ip, ua, username string) (int64, Template, error) {
	...
	err = tx.QueryRowContext(ctx, `
		...
		JOIN templates t ON t.id = i.template_id
		WHERE i.id = ?
	`, inviteID).Scan(&template.ID, &template.Name, &template.Description, &template.PolicyJSON, &template.IsDefault, &template.CreatedAt, &template.UpdatedAt)
	if errors.Is(err, sql.ErrNoRows) {
...
func (s *Store) SetInviteEnabled(ctx context.Context, id int64, enabled bool) error {
	...
	result, err := s.db.ExecContext(ctx, `UPDATE invites SET enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND deleted_at IS NULL`, value, id)
	if err != nil {
		return err
	}
	changed, err := result.RowsAffected()
...
func (s *Store) scanInvite(scanner rowScanner) (Invite, error) {
	...
	err := scanner.Scan(&invite.ID, &invite.TokenHash, &invite.TokenPrefix, &encryptedToken, &invite.Label, &invite.TemplateID, &invite.Template, &invite.ExpiresAt, &invite.MaxUses, &invite.Uses, &invite.Enabled, &invite.UserExpiryDays, &invite.LastUsedAt, &invite.DeletedAt, &invite.CreatedAt, &invite.UpdatedAt)
	if err != nil {
		return Invite{}, err
	}
	if encryptedToken != "" {
